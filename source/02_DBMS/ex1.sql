...
-- 1981년 2월 20일부터 1981년 5월 1일 사이에 입사한 사원의 사원명, 직책, 입사일을
-- 최초입사일부터 최근입사일 순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).

SELECT ENAME, JOB, HIREDATE 
    FROM EMP 
    WHERE HIREDATE BETWEEN TO_DATE('1981-02-20', 'RR/MM/DD') AND TO_DATE('1981-05-01', 'RR/MM/DD')
    ORDER BY HIREDATE;
    
-- 3.EMP테이블과 DEPT테이블의 self join과 outer join을 이용하여
-- 사원명, 급여, 부서번호, 부서명, 근무지, 직속상사명을 상사가 없는 직원까지 
-- 전체 직원 다 출력하는 SQL문을 작성하시오. 단, 상사가 없는 경우 상사명 대신 ‘없음’을 출력한다(5점).
...
-- EMP 테이블의 사원을 사번, 사원명, 직책, 급여, 급여등급을 급여 기준 내림차순으로
-- 정렬하여 출력하는 SQL문을 작성하시오(5점).
...
-- 9. 데이터베이스에서 트랜잭션은 데이터 처리의 한 단위를 말한다. 데이터를 조작하기 위한
-- INSERT, UPDATE, DELETE 등의 명령들이 실행됨과 동시에 트랜잭션이 진행되는데,
