...
INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
    VALUES (150, '원글자', '글150', '본문', 'go@go.com', '1', 150, 0, 0, '127.0.0.1');
    
INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
    VALUES ((SELECT NVL(MAX(NUM), 0)+1 FROM BOARD), '답변자', '글150-1', '본문', 'won@won.com', '1', 150, 0+1, 0+1, '127.0.0.1');

UPDATE BOARD SET RE_STEP = RE_STEP + 1 WHERE REF = 150 AND RE_STEP > 0;

INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
    VALUES ((SELECT NVL(MAX(NUM), 0)+1 FROM BOARD), '답변자2', '글150-2', '본문', 'won@won.com', '1', 150, 1, 1, '127.0.0.1');

UPDATE BOARD SET RE_STEP = RE_STEP + 1 WHERE REF = 150 AND RE_STEP > 1;

INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, RE_STEP, RE_INDENT, IP)
    VALUES ((SELECT NVL(MAX(NUM), 0)+1 FROM BOARD), '답답자', '글152-1', '본문', 'dabdab@dab.com', '1', 150, 2, 2, '127.0.0.1');
