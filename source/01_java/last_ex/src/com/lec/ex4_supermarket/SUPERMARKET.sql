...
-- 아이디 검색
SELECT CID, CTEL, CNAME, CPOINT, CAMOUNT, LEVELNAME, HIGH-CAMOUNT+1 GAP 
...
SELECT CID, CTEL, CNAME, CPOINT, CAMOUNT, LEVELNAME, HIGH-CAMOUNT+1 GAP 
    FROM CUSTOMER C, CSM_LEVEL CL WHERE C.LEVELNO = CL.LEVELNO
    AND CAMOUNT BETWEEN LOW AND HIGH AND SUBSTR(CTEL, 10, 4) = '9999';
...
SELECT CID, CTEL, CNAME, CPOINT, CAMOUNT, LEVELNAME, HIGH-CAMOUNT+1 GAP 
    FROM CUSTOMER C, CSM_LEVEL CL WHERE C.LEVELNO = CL.LEVELNO
    AND CAMOUNT BETWEEN LOW AND HIGH AND CNAME = '홍길동';

-- 포인트로 구매
UPDATE CUSTOMER SET CPOINT = CPOINT - 10000 WHERE CID = 2;

-- 물품 구매
UPDATE CUSTOMER SET CAMOUNT = CAMOUNT + 400000, CPOINT = CPOINT + (400000*0.05) WHERE CID = 5;
-- 레벨업
UPDATE CUSTOMER SET LEVELNO = (SELECT L.LEVELNO FROM CUSTOMER C, CSM_LEVEL L
                                WHERE CAMOUNT BETWEEN LOW AND HIGH AND CID=5) WHERE CID = 5;
...
-- 번호수정
UPDATE CUSTOMER SET CTEL = '010-8888-7777'
    WHERE CID = 1;
